...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/25 17:58:19 by kmazier           #+#    #+#             */
/*   Updated: 2020/12/25 17:58:19 by kmazier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	textures[index].img_ptr = mlx_xpm_file_to_image(vars->mlx_ptr, path,
		&textures[index].width,
		&textures[index].height);
	if (textures[index].img_ptr == NULL)
		return (0);
	textures[index].addr = (int*)mlx_get_data_addr(
		textures[index].img_ptr,
		&textures[index].bits_per_pixel,
		&textures[index].line_length,
		&textures[index].endian);
	return (1);
}
